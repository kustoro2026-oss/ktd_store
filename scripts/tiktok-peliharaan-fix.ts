/**
 * Bangun file upload TikTok kategori Perlengkapan Hewan Peliharaan LENGKAP
 * (semua produk yang belum diupload = 4 produk ternak).
 *
 * Sumber data:
 *  - t9-Perlengkapan Hewan Peliharaan.xlsx (baris data yang disiapkan user)
 *  - tiktok-export/products.json (stok supplier)
 *
 * Perbaikan yang dilakukan:
 *  1. URL gambar proxy wsrv.nl -> URL langsung anekadropship.id
 *     (error result: "Tautan gambar ini gagal diproses").
 *  2. Stok Gudang Jakarta (kolom 25) WAJIB utk kategori ini -> diisi dari
 *     stok supplier (products.json), fallback 100.
 *  3. Kategori "Perlengkapan Kesehatan Hewan Ternak": kolom product_property
 *     47-49, 51-55 = Forbid (dibiarkan kosong), 50 & 56 = Optional (kosong).
 *  4. Nama produk < 25 char ditambah suffix.
 *
 * Output: tiktok-upload/t9-Perlengkapan Hewan Peliharaan-FIX.xlsx
 * Jalankan: node scripts/tiktok-peliharaan-fix.ts
 */
...
const TPL = path.resolve("tiktok-upload/t9-Perlengkapan Hewan Peliharaan.xlsx");
const OUT = path.resolve("tiktok-upload/t9-Perlengkapan Hewan Peliharaan-FIX.xlsx");
...
function findSupplier(nama: string): { name: string; stock: number } | null {
  ...
  // Nama pendek -> cocokkan sebagai substring penuh
  ...
  for (const p of SUPPLIER) {
    ...
    if (score === 0) continue;
    // Hukuman jika ada angka di nama yang tidak sama (mis. "100 ml" vs "500 ml")
...
// ---- Baca baris data dari template t9 ----
...
// Baris data template: 0-based 5..10 (baris 6 contoh "Dress Malam" dibuang,
// baris 7 kategori "Obat Akuarium" kosong namanya)
for (let r = 5; r <= 10; r++) {
  ...
  // 57 kolom penuh: 0-27 tampak + 28-42 gudang lain + 43-45 SKU/order/ukuran
  // + 46 cod + 47-56 product_property.
  const row: any[] = new Array(57).fill("");
  row[0] = kat.replace(/\s*\(\d+\)\s*$/, "").trim(); // Kategori (buang "(805264)")
  row[1] = String(get(r, 1) ?? "").trim(); // Merek
  row[2] = nama.slice(0, 255); // Nama produk
  if (row[2].length < 25) row[2] = (nama + " - Perlengkapan Hewan Peliharaan Berkualitas").slice(0, 255);
  ...
  // product_property (kolom 47-56): HiddenStyle "Perlengkapan Kesehatan
  // Hewan Ternak" -> 47-49 & 51-55 Forbid (kosong), 50 & 56 Optional (kosong).
  // Tidak ada atribut wajib tambahan -> tidak diisi apa-apa.
...
// Tulis ke salinan template t9. Bersihkan dulu baris data lama (0-based >= 5)
...
for (let r = 5; r < t.length; r++) {
  ...
  console.log(
    `[${r + 1}] ${String(row[0]).slice(-35).padStart(35)} | ${String(row[2]).slice(0, 38).padEnd(38)} | berat=${row[18]} | harga=${row[23]} | stokJKT=${row[25]} | img=${img.includes("wsrv.nl") ? "WSRV-BURUK" : img.slice(-30)}`
  );
}
