/**
 * Bangun file upload TikTok kategori Perawatan & Kecantikan yang valid,
 * berdasarkan file hasil TikTok:
 *   Tiktoksellercenter_Perawatan & Kecantikan_20260910_Penghilang Bau Kaki_result.xlsx
 *
 * Alasan kegagalan dari TikTok:
 *  - "Tautan gambar ini gagal diproses" -> gambar memakai proxy wsrv.nl.
 *    Perbaikan: lepas wrapper wsrv.nl, pakai URL langsung
 *    https://anekadropship.id/uploads/products/*
 *
 * Produk yang DIKELUARKAN (kategori wajib atribut "Nomor Ijin Edar" BPOM
 * kolom 58 = product_property/101066, tidak bisa diisi tanpa nomor asli):
 *  - Meili Nutricream, Dodo Label Stiker, Underarm Whitening Cream,
 *    SCENTNICE HAIR TONIC, NAIL REVIVE, Thormax Rise Men Oil
 *  (per keputusan user: tidak punya nomor BPOM -> ditambah manual nanti)
 *
 * Perbaikan tambahan:
 *  - Garansi (kolom 47 = product_property/100107) WAJIB utk kategori
 *    "Perangkat Kecantikan Tubuh" -> diisi "Tanpa Garansi".
 *  - Stok Gudang Jakarta diisi dari stok supplier (products.json).
 *
 * Output: tiktok-upload/t11-Perawatan & Kecantikan-FIX.xlsx
 * Jalankan: node scripts/tiktok-kecantikan-fix.ts
 */
...
const TPL = path.resolve("tiktok-upload/t11-Perawatan & Kecantikan.xlsx");
const OUT = path.resolve("tiktok-upload/t11-Perawatan & Kecantikan-FIX.xlsx");
const PRODUCTS_FILE = path.resolve("tiktok-export/products.json");

// Kategori yang WAJIB "Nomor Ijin Edar" (101066) -> dikeluarkan (tanpa BPOM)
const BPOM_REQUIRED = [
  "Perawatan Kulit/Moisturiser & Mist",
  "Keperluan Mandi & Perawatan Tubuh/Cream & Lotion Tubuh",
];

// Kategori yang WAJIB garansi (100107)
const WARRANTY_REQUIRED = "Peralatan Perawatan/Perangkat Kecantikan Tubuh";
...
// ---- Baca baris data dari template t11 ----
...
for (let r = 5; r <= maxR; r++) {
  ...
  const katRaw = String(get(r, 0) ?? "").trim();
  ...
  // 62 kolom penuh: 0-27 tampak + 28-42 gudang lain + 43-45 SKU/order/ukuran
  // + 46 cod + 47-59 product_property + 60-61 qualification.
  const row: any[] = new Array(62).fill("");
  row[0] = kat;
  row[1] = String(get(r, 1) ?? "").trim(); // Merek
  row[2] = nama.slice(0, 255); // Nama produk
  if (row[2].length < 25) row[2] = (nama + " - Perawatan & Kecantikan Berkualitas").slice(0, 255);
  ...
  // Garansi (100107) wajib utk Perangkat Kecantikan Tubuh
  if (kat.endsWith(WARRANTY_REQUIRED)) row[47] = "Tanpa Garansi";
  // Nomor Ijin Edar (101066) kolom 58: tidak diisi (produk wajib BPOM sudah
  // dikeluarkan; kategori tersisa bersifat Optional/Forbid untuk kolom ini).
  // Contains dangerous goods? (101734) kolom 59: kategori tersisa = Forbid.

  rows.push(row);
  console.log(`Baris ${r + 1}: ${row[2].slice(0, 50)} -> DIPROSES (${kat.slice(-45)})`);
}
...
// Tulis ke salinan template t11. Bersihkan dulu baris data lama (0-based >= 5)
...
for (let r = 5; r < t.length; r++) {
  ...
  console.log(
    `[${r + 1}] ${String(row[0]).slice(-40).padStart(40)} | ${String(row[2]).slice(0, 36).padEnd(36)} | berat=${row[18]} | harga=${row[23]} | stokJKT=${row[25]} | garansi=${row[47]} | img=${img.includes("wsrv.nl") ? "WSRV-BURUK" : img.slice(-30)}`
  );
}
