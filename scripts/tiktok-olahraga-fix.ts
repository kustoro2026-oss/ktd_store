/**
 * Bangun file upload TikTok kategori Olahraga & Outdoor LENGKAP
 * (semua produk yang belum diupload = 2 produk).
 *
 * Sumber data:
 *  - t12-Olahraga & Outdoor.xlsx (baris data yang disiapkan user)
 ...
 *  2. Baris kosong kategori Sepakbola (nama kosong) dibuang -> itulah
 *     baris yang ditolak "Nama produk kosong" di result file.
 *  3. Stok Gudang Jakarta (kolom 25) WAJIB -> diisi dari stok supplier
 *     (products.json), fallback 100.
 *  4. HiddenStyle: "Memancing" & "Aksesori Sepatu Olahraga" tidak punya
 *     atribut wajib (kolom 47-57 Optional/Forbid) -> dibiarkan kosong.
 *  5. Nama produk < 25 char ditambah suffix.
 *
 * Output: tiktok-upload/t12-Olahraga & Outdoor-FIX.xlsx
 * Jalankan: node scripts/tiktok-olahraga-fix.ts
 */
...
function findSupplier(nama: string): { name: string; stock: number } | null {
  ...
  for (const p of SUPPLIER) {
    ...
    if (score === 0) continue;
    // Hukuman jika ada angka di nama yang tidak sama (mis. "30 ml" vs "60 ml")
...
// ---- Baca baris data dari template t12 ----
...
// Baris data template: 0-based 5..9 (baris 6 contoh "Dress Malam" dibuang,
// baris 7 kategori "Sepakbola" kosong namanya -> itulah baris yang ditolak
// TikTok "Nama produk kosong", jangan ditulis lagi).
for (let r = 5; r <= 9; r++) {
  ...
  // + 46 cod + 47-56 product_property + 57 qualification.
  const row: any[] = new Array(58).fill("");
  row[0] = kat.replace(/\s*\(\d+\)\s*$/, "").trim(); // Kategori (buang "(603818)")
  row[1] = String(get(r, 1) ?? "").trim(); // Merek
  row[2] = nama.slice(0, 255); // Nama produk
  if (row[2].length < 25) row[2] = (nama + " - Olahraga & Outdoor Berkualitas").slice(0, 255);
  ...
  // product_property (47-56) & qualification (57): HiddenStyle
  // "Memancing" -> 47,48,51,52,54 Optional, sisanya Forbid;
  // "Aksesori Sepatu Olahraga" -> 49,50,55 Optional, sisanya Forbid.
  // Tidak ada atribut wajib -> semuanya dibiarkan kosong.
...
// Tulis ke salinan template t12. Bersihkan dulu baris data lama (0-based >= 5)
